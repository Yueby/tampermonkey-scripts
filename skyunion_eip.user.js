...
(function () {
    ...
    class ToiletRoom {
        constructor(element, floorInfo, type) {
            ...
            this.onStatusChange = null;  // 添加状态变化回调属性
        }
    ...
    class ToiletManager {
        constructor() {
            ...
            this.lastNotification = null;  // 添加上一次通知的引用
            this._setupEventListeners();
        }
        ...
        _init() {
            ...
            if (this.ui.panel) {
                // 重置所有监听开关状态
                const checkboxes = this.ui.panel.querySelectorAll('.monitor-checkbox, .only-vacant-toggle');
                checkboxes.forEach(checkbox => checkbox.checked = false);
                
                // 重置筛选器
                const filters = this.ui.panel.querySelectorAll('select');
                filters.forEach(filter => filter.value = 'all');
                
                // 更新UI状态
                this.ui.currentFloor = 'all';
                this.ui.currentType = 'all';
                this.ui.onlyVacant = false;
            }

            // 初始化UI状态
            this.ui.initializeStatus(this.floors);
            console.log('UI已重置并更新');
        }
